...
public class RestAPI {
    ...
    public String search(@QueryParam("kind") int kind, @QueryParam("region") int region, @QueryParam("keywords") String keywords,@QueryParam("firstRow")int firstRow) throws IOException  {
		Document doc = Jsoup.connect(String.format("https://rent.591.com.tw/?kind=%d&region=%d&keywords=%s&firstRow=%d", kind, region, keywords, firstRow)).timeout(30000).cookie("urlJumpIp",String.valueOf(region)).userAgent("Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36").validateTLSCertificates(false).get();
